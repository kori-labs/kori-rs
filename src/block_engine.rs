...
use crate::proto::dto::MempoolTransactionsWithEffects;

pub struct KoriBlockEngine {
    block_engine_client: BlockEngineValidatorClient<Channel>,
    access_token: Option<Token>,
}

impl KoriBlockEngine {
    ...
    pub async fn stream_mempool<F>(
        &mut self,
        stream: impl Stream<Item = MempoolTransactionsWithEffects> + Send + 'static
    ...
    pub async fn subscribe_bundles<F>(
        ...
        where
            F: Fn(proto::dto::CertifiedBundle) + Send + 'static,
    {
